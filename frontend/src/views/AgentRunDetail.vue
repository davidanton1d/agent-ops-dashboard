    ...
    <div class="grid grid-cols-2 gap-6">
      ...
      <div class="bg-surface-0 border border-surface-200 rounded-lg p-4">
        ...
        <div class="space-y-2 text-sm">
          <div>
            <span class="text-surface-500">Session ID:</span>
            <span class="ml-2 font-mono text-xs">{{ run.sessionId }}</span>
          </div>
          <div>
            <span class="text-surface-500">Project:</span>
          ...
          <div v-if="run.lastOutput">
            <span class="text-surface-500">Last Output:</span>
        ...
        <div class="mt-4 pt-4 border-t border-surface-200">
          <h3 class="text-xs font-bold uppercase text-surface-500 mb-2">
            {{ run.workItemId ? 'Linked Work Item' : 'Link to Work Item' }}
          </h3>
          <div v-if="run.workItemId">
            <router-link :to="`/work-items/${run.workItemId}`" class="text-primary text-sm no-underline">
              View Work Item
            </router-link>
          </div>
          <div v-else>
            <select
              class="w-full border border-surface-200 rounded px-2 py-1 text-sm"
